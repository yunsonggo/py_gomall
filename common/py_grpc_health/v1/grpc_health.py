...
from common.py_grpc_health.v1 import grpc_health_pb2 as health_pb
...
SERVICE_NAME = health_pb.DESCRIPTOR.services_by_name['Health'].full_name
...
class HealthServicer(health_grpc.HealthServicer):
    ...
    def __init__(self,
        ...
        self._server_status = {"": health_pb.HealthCheckResponse.SERVING}
    ...
    def Check(self, request, context):
        with self._lock:
            ...
            if status is None:
                ...
                return health_pb.HealthCheckResponse(status=status)
    ...
    def Watch(self, request, context, send_response_callback=None):
        ...
        with self._lock:
            ...
            if status is None:
                status = health_pb.HealthCheckResponse.SERVICE_UNKNOWN  # pylint: disable=no-member
            send_response_callback(
                health_pb.HealthCheckResponse(status=status))
        ...
        with self._lock:
            if self._gracefully_shutting_down:
                ...
                for service in self._server_status:
                    self.set(service,
                             health_pb.HealthCheckResponse.NOT_SERVING)  # pylint: disable=no-member
